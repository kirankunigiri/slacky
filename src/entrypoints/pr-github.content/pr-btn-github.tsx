...
function PRButtonGitHub() {
	const [copyState, setCopyState] = useState<'idle' | 'copied'>('idle');

	const handleClick = async () => {
		console.log('Open PR');
		sendMessage('sendSlackMessage', { text: 'Send a message to Slack' });
	};

	const handleCopyClick = async () => {
		await copyPRMessageGitHub();
		setCopyState('copied');
		setTimeout(() => {
			setCopyState('idle');
	...
	return (
		<>
			...
			<button onClick={handleClick} className={BASE_GITHUB_BTN_CLASS}>
...
export default withStorageLoaded(PRButtonGitHub);
